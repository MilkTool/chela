...
namespace Bind.GL2
{
    class Generator : IBind
    {
        ...
        protected static string csTypemap = "chela.tm";
        ...
        protected static string delegatesFile = "GLDelegates.chela";
        protected static string enumsFile = "GLEnums.chela";
        protected static string wrappersFile = "GL.chela";
                    ...
                    do
                    {
                        ...
                        switch (words[0])
                        {
                            ...
                            case "param":   // Line denotes parameter
                                ...
                                if (p.Pointer != 0 && words.Count > 5)
                                {
                                    //&& words[5].Contains("[1]"))
                                    string[] elements = words[5].Replace('[', ' ').Replace(']', ' ').Split(Utilities.Separators, StringSplitOptions.RemoveEmptyEntries);
                                    int parsedCount = 0;
                                    if(elements.Length > 0 && int.TryParse(elements[0], out parsedCount))
                                    {
                                        //System.Console.WriteLine("element count {0}", parsedCount);
                                        p.ElementCount = parsedCount;
                                    }
                                }
        ...
        public void WriteBindings(DelegateCollection delegates, FunctionCollection functions, EnumCollection enums)
        {
            ...
            using (BindStreamWriter sw = new BindStreamWriter(temp_delegates_file))
            {
                ...
                sw.WriteLine("using Chela.Lang;");
                sw.WriteLine("using Chela.Text;");
                //sw.WriteLine("using Chela.Runtime.InteropServices;");
            ...
            // Core
            // Chela doesn't support P/Invoke.
