    ...
    public class Function : Delegate, IEquatable<Function>, IComparable<Function>
    {
        ...
        public void CreateBody(bool wantCLSCompliance)
        {
            ...
            foreach (Parameter p in f.Parameters)
            {
                ...
                else if(p.WrapperType == WrapperTypes.StringParameter)
                {
                    //TODO: Wrap strings.
                    // Use StringHandle to obtain pointer to string data
                    string pname = Utilities.Keywords.Contains(p.Name) ? "@" + p.Name : p.Name;
                    handle_statements.Add(String.Format(
                        "{0} {1}_ptr = {0}.Alloc({1});",
                        "StringHandle", pname));
                    ...
                    if(p.CurrentType.Contains("[]"))
                        p.Name = p.Name + "_ptr.ArrayAddress";
                    else
                        p.Name = p.Name + "_ptr.Address";
                }
            }
            ...
            // In Chela we are using function pointers.
